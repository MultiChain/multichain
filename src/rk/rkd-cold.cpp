...
bool AppInit(int argc, char* argv[])
{
        ...
        (mc_gState->m_Params->NetworkName() == NULL))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  rkd-cold <blockchain-name> [options]                     " + _("Start Rk Offline Daemon") + "\n";
    ...
    is_daemon=false;
        ...
        if (fDaemon)
        {
            ...
            if(!GetBoolArg("-shortoutput", false))
            {
                fprintf(stdout, "Rk server starting\n");
            }
    ...
    if(is_protocol_rk)
    {
		...
		{
		        fprintf(stderr,"ERROR: Parameter set for blockchain %s was generated by Rk running newer protocol version (%d)\n\n",
		                mc_gState->m_Params->NetworkName(),(int)mc_gState->m_NetworkParams->GetInt64Param("protocolversion"));                        
		        fprintf(stderr,"Please upgrade Rk\n\n");
		        delete mc_gState;                
		        return false;
		}
		...
		{
		        fprintf(stderr,"ERROR: The protocol version (%d) for blockchain %s has been deprecated and was last supported in Rk 1.0 beta 1\n\n",
